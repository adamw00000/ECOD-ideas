...
def run_fdr_tests(DATASET_TYPE, get_all_distribution_configs):
    for alpha in [0.05, 0.25]:
        ...
        append_mean_row(pivots['FDR'] < alpha).to_csv(os.path.join(RESULTS_DIR, f'{DATASET_TYPE}-all-FDR-alpha.csv'))
        append_mean_row(pivots['FDR'] < pivots['pi * alpha']).to_csv(os.path.join(RESULTS_DIR, f'{DATASET_TYPE}-all-FDR-pi-alpha.csv'))
